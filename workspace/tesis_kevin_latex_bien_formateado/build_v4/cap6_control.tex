...
La inferencia recorre las cuatro etapas del método de Mamdani. La \textbf{fuzzificación}
evalúa los grados de pertenencia de ICV, PI y EV. La \textbf{aplicación de reglas} obtiene el
grado de disparo de cada regla de antecedente conjuntivo mediante la t-norma del mínimo,
$\alpha_i=\min(\mu_{1},\ldots,\mu_{n})$. La \textbf{agregación} funde los consecuentes
escalados con la s-norma del máximo,
$\mu_{\text{agregado}}(\Delta T)=\max_i\big(\alpha_i\cdot\mu_{\text{cons},i}(\Delta T)\big)$.
Por último, la \textbf{defuzzificación} por centroide devuelve el valor nítido:
\[
\Delta T_{verde}^{*}=
