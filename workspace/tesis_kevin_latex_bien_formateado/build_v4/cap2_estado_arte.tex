...
vehicular y actuar las señales con eficiencia. Durante décadas, esa medición recayó en el lazo inductivo, un
devanado de cable embebido en la calzada cuyo principio es electromagnético: cuando un vehículo —masa
metálica— se sitúa sobre él, modifica la inductancia del devanado y delata su presencia (Buchanan, 2023). Su
exactitud y su respuesta casi instantánea son notables, pero el precio es alto, pues obliga a romper el
pavimento para instalarlo y se degrada cada vez que la vía se repavimenta.
...
Del lado de los actuadores, la señalización actual se apoya en tecnología LED, que recorta el consumo en torno
a un 80--90\,\%, alcanza vidas útiles del orden de 50\,000 horas y se percibe con mayor nitidez; como ventaja
añadida, los módulos con diodos redundantes siguen mostrando la indicación aunque uno de los diodos falle, lo
que refuerza la seguridad (Buchanan, 2023). Los controladores de corriente que encienden las luces deben
...
clasificación, velocidad y longitud de cola. Aun con poco cómputo, las
técnicas clásicas de sustracción de fondo conservan utilidad: la Mezcla de Gaussianas (MOG2) describe cada
píxel como una suma de gaussianas que se reajustan ante variaciones lentas de iluminación y mantiene buena
precisión en escenas urbanas controladas (Zivkovic, 2004). El descriptor HOG
(\emph{Histogram of Oriented Gradients}) rinde bien al detectar peatones en los cruces, si bien se resiente
ante las oclusiones parciales tan habituales cuando la intersección se congestiona (Dalal y Triggs, 2005).
Por su parte, los clasificadores Haar Cascade resultan muy livianos y, acoplados a un seguidor, estiman la
velocidad con error bajo, aunque elevan la tasa de falsos positivos (Viola y Jones, 2001).

El aprendizaje profundo relegó a estas técnicas clásicas y se erigió en la referencia para la detección en
tiempo real. Los detectores de la familia YOLO (\emph{You Only Look Once}) resuelven la localización y la
clasificación en una única inferencia sobre la imagen, lo que les permite sostener una precisión alta a tasas
de cuadro compatibles con el tráfico; por esa razón se han vuelto la herramienta más difundida para contar y
clasificar vehículos en vídeo (Redmon et al., 2016). La detección
cuadro a cuadro se complementa con algoritmos de seguimiento que mantienen identidades consistentes a lo
largo del tiempo; Deep SORT (\emph{Simple Online and Realtime Tracking with Deep Association Metric}) suma a
la predicción de posición de un filtro de Kalman una huella de apariencia que una red neuronal extrae de cada
objeto; ese rasgo visual disminuye de forma marcada los intercambios de identidad que sufre un seguimiento
basado solo en cinemática (Wojke et al., 2017). Esta combinación de detección y seguimiento es la que habilita la estimación de flujo, cola y
...
máximo predefinido mientras persistan vehículos. El método de Webster (1958) sentó las bases del temporizado
optimizado al expresar el ciclo de mínima demora como $C_0 = (1{,}5L + 5)/(1 - Y)$, con $L$ el tiempo total
perdido e $Y$ la suma de las razones de flujo crítico; su punto débil aparece cuando la relación
volumen/capacidad se acerca a la unidad, pues la fórmula infla el ciclo y arroja duraciones poco realistas
que empeoran el desempeño en saturación (Webster, 1958). El refinamiento de Akçelik (1981) amplió este marco analítico con una
...
Sobre ese cimiento aparecieron los sistemas adaptativos comerciales. SCATS (\emph{Sydney Coordinated Adaptive
Traffic System}) recalibra ciclo a ciclo la duración total y el reparto de verdes a partir de la saturación
medida en la línea de parada (Fehon, 2016); SCOOT (\emph{Split Cycle Offset Optimization Technique}), en
cambio, introduce ajustes finos en cada ciclo con detectores aguas arriba y afina los desfases para encadenar
olas verdes progresivas. Ambos son de mando centralizado y acreditan menos demora y más capacidad, pero a
cambio reclaman una calibración minuciosa y una inversión nada menor en detección y comunicaciones. Como alternativa descentralizada, Surtrac (\emph{Scalable Urban Traffic Control}) hace que
...
elige en cada paso la fase cuya activación maximiza el desbalance de presión entre los enlaces que entran y
los que salen del cruce; la política es descentralizada —cada intersección decide por sí sola— y posee
garantía teórica de estabilidad de las colas; las
...
desempeño elevado con interpretabilidad. Un sistema difuso traduce magnitudes numéricas en grados de
pertenencia a etiquetas lingüísticas —\emph{tráfico pesado}, \emph{cola larga}, \emph{tiempo de espera
excesivo}— por medio de funciones de membresía, y encadena reglas \emph{si--entonces} para resolver si
conviene extender o conmutar la fase. El esquema imita el modo en que razona un ingeniero de tránsito y,
frente a los modelos de caja negra, deja cada regla a la vista para inspeccionarla, validarla y certificarla
antes de operar; esa transparencia es determinante cuando la autoridad final de seguridad permanece en manos
del controlador.
...
Llevar la Internet de las Cosas (IoT) al control de tráfico aporta conectividad y gestión remota de los
semáforos. En la práctica, un despliegue IoT añade a cada controlador un módulo que percibe su entorno, se
comunica y queda administrable desde la nube. La comunicación con la
...
está ampliamente soportado por los controladores modernos (NTCIP, 2025). Para la telemetría IoT se recurre a
MQTT (\emph{Message Queuing Telemetry Transport}), cuyo modelo de publicación/suscripción y sus cabeceras
ligeras lo hacen idóneo para cursar lecturas de sensores con alta frecuencia de actualización y entrega
confiable sobre redes urbanas (Banks y Gupta, 2014). Por encima de ambos, TCP/IP actúa como capa de transporte de red
...
Las defensas habituales se despliegan por capas. El cifrado de extremo a extremo, junto con los protocolos de
transporte (TLS/SSL), preserva la confidencialidad y la integridad de los datos en tránsito y habilita la
autenticación mutua entre dispositivos. La autenticación multifactor (MFA) y el control de acceso por roles
(RBAC) reservan el ingreso al personal habilitado y recortan lo que cada rol puede hacer. Las firmas
digitales dan fe del origen y de la integridad de cada mensaje, mientras que las redes privadas virtuales
(VPN) y los cortafuegos aíslan el tráfico de amenazas externas. Por último, auditar las vulnerabilidades con
regularidad y parchear sin demora sostiene la protección frente a amenazas nuevas.
...
SUMO (\emph{Simulation of Urban Mobility}) es un microsimulador de código abierto, muy asentado en la
academia y capaz de escalar a redes extensas. A su carácter gratuito y altamente configurable suma el módulo
TraCI (\emph{Traffic Control Interface}), que abre un canal en vivo con la simulación en marcha: un proceso
externo —el propio módulo de control IoT, por ejemplo— puede leer el estado del tráfico e inyectar acciones,
como forzar el cambio de una luz, mientras la corrida avanza. Esto lo
