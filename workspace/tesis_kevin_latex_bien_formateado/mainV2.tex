...
La tesis diseña y establece un procedimiento de validación en entorno simulado para un módulo IoT mecatrónico complementario al controlador semafórico existente. El módulo integra una cámara para estimar variables de tráfico, procesamiento local en el borde, cálculo de un índice de congestión, control difuso para generar parámetros de fase recomendados, comunicación segura con una plataforma en la nube y medidas de protección de la información. El controlador semafórico existente conserva la autoridad final sobre luces, enclavamientos, fases incompatibles, tiempos de despeje y modos seguros; por tanto, la propuesta no sustituye un controlador certificado ni acciona directamente estados críticos.

El desarrollo sigue VDI 2221 para la clarificación de la tarea, definición de requerimientos, estructura funcional, matriz morfológica y selección técnico-económica; VDI 2206 se emplea para integrar los dominios mecánico, electrónico, de control, comunicaciones y software. La verificación se plantea mediante revisión de diseño, cálculos eléctricos y mecánicos, análisis de amenazas y simulaciones reproducibles en SUMO que comparan control fijo y control difuso adaptativo. Los resultados cuantitativos de desempeño deben incorporarse únicamente desde los archivos de salida del simulador en los campos señalados como pendientes. El trabajo constituye una base técnica trazable para fabricar un prototipo y ejecutar un piloto posterior; no incluye instalación vial, pruebas con tráfico real, certificación de seguridad funcional ni auditoría formal de ciberseguridad.
...
La propuesta presentada en esta tesis aborda estos desafíos como un problema de integración mecatrónica. El sistema debe observar el estado de la intersección, transformar mediciones en variables de tráfico, calcular ajustes dentro de límites operativos, conservar capacidad local ante pérdida de conectividad y comunicar telemetría sin exponer la seguridad funcional del controlador vial. Para ello se diseña un módulo IoT complementario con visión por computadora, procesamiento en el borde, control difuso y una plataforma en la nube destinada a supervisión, almacenamiento histórico y distribución controlada de parámetros.
...
Se propone diseñar un módulo IoT complementario por intersección. La cámara constituye el sensor principal; el procesamiento local extrae variables de tráfico y calcula un índice de congestión; el controlador difuso transforma dichas variables en ajustes recomendados de tiempo verde dentro de límites mínimos, máximos y de despeje. Estas recomendaciones se entregan por una interfaz definida y deben ser aceptadas, rechazadas o acotadas por el controlador semafórico o por la central autorizada. La propuesta no conecta una salida de propósito general directamente a las lámparas.

La plataforma en la nube recibe telemetría, estados, alertas y registros; conserva históricos y puede emitir parámetros de configuración o recomendaciones de coordinación previamente autorizados. La operación local no depende de una respuesta de la nube. Si se pierde conectividad, el módulo conserva la última configuración válida, registra eventos localmente y continúa en modo degradado; si el sensado pierde calidad, deja de generar ajustes adaptativos y solicita el retorno al plan base del controlador.

La solución se valida como diseño mecatrónico mediante requerimientos trazables, selección de alternativas, cálculos de alimentación y protección, diseño de gabinete y soporte, definición del control difuso, análisis de amenazas y comparación reproducible en SUMO. Por tanto, el aporte demostrable es una arquitectura de modernización gradual y un procedimiento de verificación; cualquier mejora de tráfico debe sustentarse con los resultados cuantitativos obtenidos en los escenarios de simulación.

\section{Objetivos de la tesis}\label{objetivos-del-proyecto-de-tesis}

\subsection{Objetivo general}\label{objetivo-general}

Diseñar y validar en un entorno simulado y controlado un módulo IoT complementario para control semafórico adaptativo en intersecciones urbanas, integrando sensado vehicular, procesamiento local, lógica difusa, comunicación segura con la nube y criterios de ciberseguridad, manteniendo al controlador semafórico existente como autoridad final de seguridad funcional.

\subsection{Objetivos específicos}\label{objetivos-especuxedficos}

\begin{enumerate}[label=\textbf{OE\arabic*:},leftmargin=1.7cm]
\item Revisar el estado del arte de control semafórico, sensado vehicular, procesamiento en el borde, comunicaciones IoT, nube, ciberseguridad y simulación, y consolidar las decisiones de diseño derivadas en una tabla de criterios adoptados.
\item Definir requerimientos funcionales, electrónicos, mecánicos, de control, comunicación, operación, mantenimiento, ciberseguridad, validación y costo, indicando para cada uno su criterio y método de verificación.
\item Desarrollar el diseño conceptual mediante caja negra, entradas y salidas, estructura funcional por dominios, matriz morfológica y evaluación técnico-económica de alternativas, hasta seleccionar una arquitectura trazable.
\item Diseñar el subsistema electrónico del módulo IoT, justificando sensores, unidad de procesamiento, interfaces, comunicaciones, alimentación, protecciones, diagnóstico y respaldo energético mediante esquemas, cálculos y tablas de selección.
\item Diseñar el subsistema mecánico, incluyendo gabinete modular, distribución interna, protección ambiental, acceso de mantenimiento y soporte o movimiento de cámara, y verificarlo mediante CAD, cálculos y análisis estructural/térmico.
\item Diseñar el control adaptativo local basado principalmente en lógica difusa, definiendo entradas, salida, funciones de pertenencia, reglas, inferencia, defuzzificación, restricciones de fase y modos degradados.
\item Diseñar la arquitectura de comunicación y nube para telemetría, históricos, alertas, coordinación y actualización autorizada de parámetros, demostrando que la decisión local puede continuar sin conectividad externa.
\item Diseñar una arquitectura de ciberseguridad mediante modelo de amenazas, autenticación, cifrado, control de acceso, validación de parámetros, registro de eventos y principio de mínimo privilegio.
\item Validar en SUMO el núcleo de control mediante escenarios reproducibles que comparen, como mínimo, control fijo y control difuso adaptativo, reportando tiempo de espera, colas, velocidad, throughput, paradas, tiempo de viaje, índice de congestión y, cuando esté disponible, consumo o emisiones.
\item Elaborar un análisis económico preliminar de CAPEX y OPEX por intersección, diferenciando costos del módulo local, conectividad, nube, mantenimiento y etapas posteriores de certificación o piloto.
\end{enumerate}

\section{Alcance del trabajo}\label{alcance-del-trabajo}

El presente trabajo corresponde a una tesis de diseño mecatrónico enfocada en el diseño, simulación y validación en entorno controlado de un módulo IoT complementario para control semafórico adaptativo. Incluye: diseño conceptual; especificación de requerimientos; arquitectura funcional; diseño electrónico y mecánico; arquitectura de comunicación y nube; arquitectura de ciberseguridad; algoritmo de control difuso local; integración con una plataforma controlada de supervisión; simulación comparativa en SUMO; y análisis económico preliminar CAPEX/OPEX por intersección.
...
El trabajo no incluye fabricación física, instalación en vía pública, certificación de seguridad funcional, auditoría formal de ciberseguridad, pruebas con tráfico real, integración certificada con controladores comerciales específicos ni despliegue metropolitano completo. La compatibilidad con marcas y protocolos presentes en Lima se formula como una arquitectura de integración y debe verificarse posteriormente con equipos propietarios. Del mismo modo, la referencia a IEC 62443 y a prácticas de seguridad expresa criterios de diseño, no conformidad certificada.

Estas exclusiones corresponden a etapas posteriores de madurez tecnológica: fabricación de prototipo, banco de integración hardware-in-the-loop, piloto controlado, evaluación normativa y certificación. El alcance geográfico se mantiene en intersecciones representativas modeladas en simulación; la extrapolación a corredores o a la red metropolitana requerirá calibración con aforos reales, validación operacional, acuerdos institucionales y pruebas progresivas.

\section{Metodología empleada}\label{metodologuxeda-empleada}

Para el desarrollo de la tesis se emplea VDI 2221 como marco principal de diseño sistemático y VDI 2206 para ordenar la integración y verificación de los dominios mecánico, electrónico, de control y software. Design Thinking se utiliza únicamente como apoyo inicial para identificar necesidades de usuarios y operadores; no reemplaza la especificación técnica ni el proceso de validación.
...
\begin{enumerate}
\item \textbf{Clarificación de la tarea:} formulación del problema técnico, objetivos, alcance, actores y necesidades.
\item \textbf{Especificación:} requerimientos codificados con criterio, prioridad, justificación y método de verificación.
\item \textbf{Diseño conceptual:} caja negra, entradas/salidas, estructura funcional por dominios, matriz morfológica, conceptos y evaluación ponderada.
\item \textbf{Diseño de detalle:} selección y dimensionamiento de electrónica, energía, gabinete, soporte de cámara, interfaces y comunicaciones.
\item \textbf{Diseño de control y seguridad:} definición del controlador difuso, restricciones operativas, modos degradados, arquitectura edge--nube y modelo de amenazas.
\item \textbf{Integración y validación:} escenarios SUMO reproducibles, comparación contra línea base, prueba simulada de pérdida de conectividad y matriz de cumplimiento de requerimientos.
\item \textbf{Evaluación económica y cierre:} CAPEX/OPEX, conclusiones vinculadas a objetivos, limitaciones y plan de implementación futura.
\end{enumerate}

Cada objetivo específico se relaciona con requerimientos, capítulos y evidencias mediante una matriz de trazabilidad. Un requerimiento se considera verificado únicamente cuando existe una figura, tabla, cálculo, archivo de simulación o resultado identificable. Cuando la evidencia todavía no está disponible se utiliza el marcador \textbf{[COMPLETAR CON RESULTADO O EVIDENCIA]}, evitando presentar como validado aquello que solo ha sido diseñado o propuesto.
...
\subsection{Especificación verificable del sistema}\label{especificacion-verificable}

La lista anterior resume deseos y exigencias generales. Para conducir el diseño y cerrar la validación se establece la Tabla 3.2 como especificación verificable. Los valores indicados como propuestos deben confirmarse mediante cálculo, normativa aplicable o simulación antes de una implementación piloto. La prioridad se clasifica como M (mandatoria), A (alta) o D (deseable).

\begin{landscape}
\footnotesize
\captionof*{table}{\small\bfseries Tabla 3.2. Requerimientos verificables del módulo IoT semafórico. (Creación propia)}
\addcontentsline{lot}{table}{Tabla 3.2. Requerimientos verificables del módulo IoT semafórico. (Creación propia)}
\label{tbl:req-verificables}
\begin{longtable}{@{}p{1.15cm}p{1.65cm}p{5.25cm}p{3.15cm}p{4.35cm}p{3.65cm}p{1.25cm}@{}}
\toprule
\textbf{Código} & \textbf{Categoría} & \textbf{Requerimiento} & \textbf{Valor objetivo o criterio} & \textbf{Justificación} & \textbf{Método de verificación} & \textbf{Prioridad}\\
\midrule
\endfirsthead
\toprule
\textbf{Código} & \textbf{Categoría} & \textbf{Requerimiento} & \textbf{Valor objetivo o criterio} & \textbf{Justificación} & \textbf{Método de verificación} & \textbf{Prioridad}\\
...
R-C03 & Control & Respetar límites mínimos, máximos y tiempos de despeje para cada fase. & Valores propuestos en el Capítulo 6; \textbf{[VALIDAR CON NORMA Y CONTROLADOR OBJETIVO]}. & Impide recomendaciones fuera del dominio operacional permitido. & Revisión de reglas, saturaciones y barrido de entradas. & M\\
...
R-COM01 & Comunicación & Transmitir telemetría, métricas de tráfico, alertas y estado del equipo hacia la plataforma de supervisión. & MQTT/TLS u otro protocolo equivalente; frecuencia \textbf{[DEFINIR]}. & La nube cumple funciones de supervisión e histórico, no de seguridad funcional. & Revisión de tópicos, carga útil y prueba de intercambio. & A\\
...
R-NUB01 & Nube & Almacenar históricos, mostrar estado, gestionar alertas y apoyar coordinación no crítica. & Disponibilidad objetivo propuesta \textbf{[DEFINIR SEGÚN PILOTO]}. & Centraliza análisis y operación sin cerrar el lazo crítico en la nube. & Demostración en entorno controlado y revisión de arquitectura. & A\\
...
R-ELEC01 & Electrónico & Incorporar protecciones contra sobrecorriente, sobretensión y fallas de alimentación. & Protecciones dimensionadas; valores \textbf{[VALIDAR CON CÁLCULO]}. & El entorno vial presenta transitorios y operación continua. & Esquema eléctrico, hojas de datos y coordinación de protecciones. & M\\
R-ELEC02 & Electrónico & Disponer de respaldo energético para apagado controlado o continuidad temporal del módulo. & Autonomía propuesta \textbf{[CALCULAR CON PERFIL DE CARGA]}. & Evita pérdida abrupta de registros y diagnósticos. & Balance de potencia y cálculo de batería/UPS. & A\\
...
R-MEC01 & Mecánico & Alojar los componentes con acceso de mantenimiento, ventilación y reserva de expansión. & Gabinete modular; volumen y holguras \textbf{[VALIDAR EN CAD]}. & La mantenibilidad condiciona el costo de operación. & CAD, plano de distribución y procedimiento de acceso. & A\\
R-MEC02 & Mecánico & Proteger los componentes frente a polvo, humedad, radiación y manipulación. & IP objetivo propuesto y resistencia al impacto \textbf{[DEFINIR/VALIDAR]}. & La instalación prevista es exterior y accesible. & Especificación de gabinete, sellos, material y análisis de riesgos. & M\\
R-MEC03 & Mecánico & Posicionar la cámara con cobertura de las zonas de interés sin comprometer estabilidad. & Campo visual y recorrido \textbf{[VALIDAR CON GEOMETRÍA Y PRUEBA]}. & La calidad de estimación depende del encuadre y la oclusión. & CAD, cálculo de torque, cobertura y simulación estructural. & A\\
...
R-COST01 & Costos & Estimar CAPEX y OPEX por intersección, separando módulo, instalación futura, conectividad, nube y mantenimiento. & Valores con fuente, fecha y supuestos; \textbf{[ACTUALIZAR COTIZACIONES]}. & Permite evaluar viabilidad y escalamiento. & Hoja de costos y análisis de sensibilidad. & A\\
\end{longtable}
\end{landscape}
...
\subsection{Definición formal de entradas y salidas}\label{entradas-salidas-formales}

La frontera del sistema se fija alrededor del módulo IoT complementario. El controlador semafórico existente y la plataforma en la nube son sistemas externos con los que se intercambian señales; las lámparas y sus enclavamientos permanecen bajo responsabilidad del controlador vial.

\begin{center}
\fbox{\begin{minipage}{0.96\linewidth}
\centering
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}X c >{\raggedright\arraybackslash}X}
\textbf{Entradas} & \textbf{Módulo IoT complementario} & \textbf{Salidas}\\
\midrule
Video de cámara; sensores auxiliares; estado del controlador; parámetros autorizados; históricos disponibles; restricciones de seguridad; estado de conectividad; energía. &
\(\longrightarrow\) Captar, estimar, decidir, validar, registrar y comunicar \(\longrightarrow\) &
Índice de congestión; parámetros de fase recomendados; telemetría; alertas; reportes; estado del módulo; solicitudes a la central/controlador; históricos almacenados.
\end{tabularx}
\end{minipage}}
\captionof*{figure}{\small\itshape Figura 3.A. Caja negra y frontera de responsabilidad del módulo IoT. (Creación propia)}
\addcontentsline{lof}{figure}{Figura 3.A. Caja negra y frontera de responsabilidad del módulo IoT. (Creación propia)}
\label{fig:caja-negra-v2}
\end{center}

\begin{longtable}{@{}p{0.19\linewidth}p{0.34\linewidth}p{0.39\linewidth}@{}}
\toprule
...
Entrada & Sensores auxiliares y diagnóstico eléctrico/ambiental & Detectar condiciones internas anómalas y apoyar mantenimiento.\\
...
Entrada & Restricciones de seguridad y conectividad & Seleccionar modo normal, degradado o retorno al plan base.\\
...
Salida & Telemetría, alertas, reportes y estado & Operación, mantenimiento, auditoría y análisis histórico.\\
Salida & Registro local e histórico remoto & Evidencia para trazabilidad, diagnóstico y calibración.\\
\bottomrule
\end{longtable}

\subsection{Descomposición funcional por dominios}\label{dominios-funcionales-v2}

La estructura funcional se divide en nueve dominios: \textbf{sensado}, que capta la escena vial; \textbf{procesamiento local}, que preprocesa y estima variables; \textbf{control}, que calcula el ajuste difuso y aplica restricciones; \textbf{comunicación}, que intercambia mensajes; \textbf{nube}, que supervisa y conserva históricos; \textbf{ciberseguridad}, que protege identidad, acceso, integridad y registros; \textbf{energía}, que alimenta, convierte, respalda y protege; \textbf{mecánico}, que soporta y protege sensores/electrónica; y \textbf{operación y mantenimiento}, que diagnostica, actualiza y recupera el sistema.

Las funciones internas mínimas son: captar la escena vial; preprocesar datos; estimar flujo, cola, velocidad u ocupación; calcular el índice de congestión; ejecutar control difuso local; validar restricciones de seguridad; comunicar telemetría; recibir parámetros autorizados; registrar eventos; gestionar fallos de comunicación y sensado; proteger accesos y datos; alimentar y proteger componentes; y soportar físicamente la cámara y la electrónica.
...
\section{Matriz morfológica y selección de alternativa}\label{matriz-morfologica-v2}

La matriz morfológica vincula cada función principal con tecnologías capaces de realizarla. Su propósito no es listar componentes de forma aislada, sino generar conceptos completos y comparables. La alternativa seleccionada se resalta en negrita y se evalúa posteriormente como parte del concepto C.

\begin{landscape}
\footnotesize
\captionof*{table}{\small\bfseries Tabla 3.3. Matriz morfológica del sistema. (Creación propia)}
\addcontentsline{lot}{table}{Tabla 3.3. Matriz morfológica del sistema. (Creación propia)}
\label{tbl:matriz-morfologica-v2}
\begin{longtable}{@{}p{4.0cm}p{3.8cm}p{3.8cm}p{3.8cm}p{3.8cm}p{3.8cm}@{}}
\toprule
\textbf{Función} & \textbf{Alternativa 1} & \textbf{Alternativa 2} & \textbf{Alternativa 3} & \textbf{Alternativa 4} & \textbf{Alternativa 5}\\
\midrule
\endhead
Sensar tráfico & \textbf{Cámara RGB} & Radar & Lazo inductivo & Ultrasónico/LiDAR & API externa\\
...
Proteger físicamente & Caja comercial interior & Gabinete plástico & \textbf{Gabinete metálico modular exterior} & Rack abierto & Gabinete de controlador existente\\
Validar control & Cálculo manual & \textbf{SUMO/TraCI} & VISSIM & AIMSUN & Prueba en vía pública\\
\bottomrule
\end{longtable}
\end{landscape}

\subsection{Conceptos generados}\label{conceptos-generados-v2}

\begin{itemize}
\item \textbf{Concepto A --- control centralizado en nube:} cámaras económicas transmiten datos hacia la nube, donde se calcula el control. Presenta alta escalabilidad informática, pero dependencia crítica de conectividad y mayor exposición de datos.
\item \textbf{Concepto B --- solución industrial local cerrada:} radar o lazos, PLC/controlador industrial y red cableada, con mínima participación de nube. Ofrece alta robustez local, pero eleva costo de obra e integración y limita la experimentación académica.
\item \textbf{Concepto C --- módulo complementario edge--nube:} cámara, procesamiento local, índice de congestión, control difuso, comunicación segura con nube, gabinete modular y controlador vial como autoridad final.
\item \textbf{Concepto D --- control directo basado en aprendizaje:} visión y aprendizaje por refuerzo generan acciones de fase. Su potencial es alto, pero exige datos, validación y garantías que exceden el alcance y no ofrece la interpretabilidad requerida como núcleo de esta tesis.
\end{itemize}

\subsection{Evaluación técnico-económica ponderada}\label{evaluacion-ponderada-v2}
...
\begin{landscape}
\footnotesize
\captionof*{table}{\small\bfseries Tabla 3.4. Evaluación ponderada de conceptos de solución. (Creación propia)}
\addcontentsline{lot}{table}{Tabla 3.4. Evaluación ponderada de conceptos de solución. (Creación propia)}
\label{tbl:evaluacion-conceptos-v2}
...
El concepto C obtiene la mayor puntuación porque combina autonomía local, interpretabilidad, compatibilidad con validación en SUMO, supervisión remota y separación de responsabilidades. Por tanto, la solución seleccionada es: \textbf{cámara + procesamiento local en el borde + índice de congestión + control difuso + comunicación segura con nube + SUMO como simulador + gabinete modular + arquitectura complementaria al controlador existente}. Esta elección no implica que cada componente comercial ya esté certificado; indica que el concepto satisface mejor los requerimientos del alcance de tesis.

\section{Solución óptima}\label{soluciuxf3n-uxf3ptima}

La solución óptima corresponde al concepto C de la evaluación precedente. Se trata de un módulo mecatrónico instalado en la intersección, con cámara y gabinete de protección, cuya ubicación y altura deben definirse mediante análisis de cobertura, oclusiones, accesibilidad y resistencia del soporte. La altura de 6,5 m y el grado IP65 se consideran objetivos de diseño propuestos y deben confirmarse en la etapa de integración con la infraestructura real. Una visualización del concepto se presenta en la Figura 47.
...
La alimentación del sistema se diseña mediante una arquitectura que incluye llaves termomagnéticas, protección diferencial o equivalente según la instalación, una fuente AC/DC y convertidores DC/DC para los niveles requeridos. El respaldo mediante batería o UPS debe dimensionarse con el perfil de carga; por ello, la autonomía solo podrá afirmarse después de completar el cálculo y la prueba correspondiente.
...
El procesamiento local se plantea con OpenCV y un detector de objetos ejecutado en la unidad edge, con el fin de estimar flujo, velocidad y cola. La detección de vehículos de emergencia se considera una función complementaria cuya precisión debe validarse con un conjunto de datos específico. El acondicionamiento prepara telemetría para la central y la nube. Microsoft Azure se emplea como plataforma de referencia para almacenamiento, supervisión y recomendaciones de coordinación, sin eliminar la validación local ni la autoridad final del controlador.

La arquitectura de ciberseguridad propuesta considera cifrado en tránsito, gestión de identidades y accesos, MFA para operadores, registro de eventos y separación de redes o zonas de confianza. Su efectividad debe verificarse en un banco de integración y mediante pruebas autorizadas; el presente diseño no acredita una certificación.
...
La detección y seguimiento de vehículos de emergencia se plantea como módulo complementario: cámara, estimación de trayectoria y generación de una solicitud de prioridad para revisión o validación por la capa autorizada. No se considera parte del criterio mínimo de cumplimiento del controlador difuso hasta disponer de un conjunto de prueba, métricas de detección y reglas operativas de prioridad.
...
En la figura 48 se aprecia el flujo general. El controlador vial acciona las salidas de campo mediante sus interfaces eléctricas y conserva los enclavamientos. NTCIP sobre TCP/IP se considera para el intercambio de estado y parámetros con la central o el módulo únicamente cuando el equipo instalado lo soporte; no es el enlace eléctrico hacia las lámparas. El módulo IoT recopila datos locales, genera recomendaciones acotadas y comunica telemetría con la nube mediante MQTT/TLS.
...
\subsection{Jerarquía de alcance y responsabilidad}\label{jerarquia-alcance-v2}

\begin{center}
\fbox{\begin{minipage}{0.96\linewidth}
\centering
\textbf{Cámara y sensores} \(\rightarrow\) \textbf{Edge: estimación + control difuso + validación}\\
\(\downarrow\) telemetría segura \hspace{1.5cm} \(\downarrow\) recomendación acotada\\
\textbf{Nube: supervisión, históricos y coordinación} \hspace{0.6cm} \textbf{Controlador semafórico: autoridad final}\\
\(\uparrow\) parámetros autorizados \hspace{2.2cm} \(\downarrow\) luces y enclavamientos seguros
\end{minipage}}
\captionof*{figure}{\small\itshape Figura 3.B. Relación edge--nube--controlador y frontera de seguridad funcional. (Creación propia)}
\addcontentsline{lof}{figure}{Figura 3.B. Relación edge--nube--controlador y frontera de seguridad funcional. (Creación propia)}
\label{fig:edge-nube-controlador-v2}
\end{center}
...
Nivel 4: módulos exploratorios/futuros & Aprendizaje por refuerzo, CNN--LSTM, optimización metropolitana, reentrenamiento continuo e integración comercial certificada. & Propuesta o experimento separado; no condiciona el cumplimiento del objetivo general.\\
\bottomrule
\end{longtable}

\section{Matriz de trazabilidad de objetivos}\label{trazabilidad-objetivos-v2}

\begin{landscape}
\footnotesize
\captionof*{table}{\small\bfseries Tabla 3.5. Trazabilidad entre objetivos, requerimientos y evidencia. (Creación propia)}
\addcontentsline{lot}{table}{Tabla 3.5. Trazabilidad entre objetivos, requerimientos y evidencia. (Creación propia)}
\label{tbl:trazabilidad-objetivos-v2}
\begin{longtable}{@{}p{1.4cm}p{4.3cm}p{4.2cm}p{3.7cm}p{7.0cm}@{}}
\toprule
...
OE2 & R-F01 a R-COST01 & 3 & Inspección de especificación & Tabla 3.2 con código, criterio, justificación y verificación.\\
...
OE9 & R-VAL01 a R-VAL03 & 7 & Simulación comparativa & \textbf{[COMPLETAR CON RESULTADOS SUMO Y ARCHIVOS DE SALIDA]}.\\
...
\section{Criterios de selección y relación con requerimientos}\label{criterios-electronicos-v2}

La selección electrónica se realiza por función y requerimiento, no solo por disponibilidad comercial. Para cada elemento se identifica la alternativa considerada, la razón de elección, su limitación principal y el modo de falla que debe tratarse en la integración.

\begin{landscape}
\footnotesize
\captionof*{table}{\small\bfseries Tabla 4.0. Selección trazable de componentes electrónicos. (Creación propia)}
\addcontentsline{lot}{table}{Tabla 4.0. Selección trazable de componentes electrónicos. (Creación propia)}
\label{tbl:seleccion-electronica-v2}
\begin{longtable}{@{}p{3.1cm}p{4.2cm}p{4.1cm}p{3.4cm}p{5.2cm}p{3.0cm}@{}}
\toprule
...
\midrule
\endhead
Sensado de tráfico & Cámara RGB, radar, lazo, LiDAR & Información obtenida, obra civil, costo, mantenimiento & Cámara exterior & Entrega múltiples variables sin intervenir pavimento; sensible a oclusión e iluminación. Ante baja calidad debe inhibirse la adaptación. & R-F01, R-C04\\
...
Comunicación & Ethernet, Wi-Fi, 4G/LTE, fibra & Cobertura, independencia, costo y disponibilidad & 4G/LTE y/o Ethernet disponible & Facilita despliegue; la pérdida de enlace no debe detener el control local. & R-COM01, R-VAL03\\
Monitoreo eléctrico & Shunt, efecto Hall, medidor digital & Aislamiento, rango, precisión, interfaz & Sensor aislado o medidor digital compatible & Permite diagnóstico; una medición fuera de rango debe generar alerta, no una decisión de fase. & R-ELEC03\\
Fuente principal & Fuente abierta, industrial DIN, ATX & Protección, eficiencia, temperatura, mantenibilidad & Fuente industrial AC/DC & Adecuada para operación continua; debe dimensionarse con margen y coordinación de protecciones. & R-ELEC01\\
Conversión DC/DC & Lineal, buck comercial, módulo industrial & Eficiencia, ruido, rango y protección & Convertidores buck regulados & Reduce pérdidas; requiere filtrado y separación de cargas sensibles/motores. & R-ELEC01, R-ELEC03\\
Respaldo & Sin respaldo, power bank, UPS, batería DC & Autonomía, seguridad, mantenimiento & UPS/batería dimensionada & Mantiene registro y apagado controlado; autonomía pendiente de perfil real de carga. & R-ELEC02\\
Interfaz con controlador & Relé directo, GPIO, protocolo estándar, API propietaria & Seguridad, aislamiento, compatibilidad, autoridad final & Interfaz validada y aislada; NTCIP cuando sea compatible & No se autoriza mando directo de lámparas. Falla o incompatibilidad implica retorno a solo supervisión. & R-C02, R-COM02\\
\bottomrule
\end{longtable}
\end{landscape}
...
\section{Requerimientos mecánicos verificables}\label{requerimientos-mecanicos-v2}

El gabinete y el soporte de cámara son subsistemas funcionales: condicionan la calidad del sensado, la temperatura de la electrónica, la mantenibilidad y la resistencia a manipulación. La Tabla 5.0 concentra los criterios que deben demostrarse mediante CAD, cálculo, simulación o especificación del fabricante.

\begin{longtable}{@{}p{0.15\linewidth}p{0.27\linewidth}p{0.25\linewidth}p{0.25\linewidth}@{}}
\toprule
\textbf{Código} & \textbf{Criterio} & \textbf{Objetivo de diseño} & \textbf{Verificación}\\
\midrule
\endhead
RM-01 & Resistencia estructural & Soportar peso propio, componentes, viento, vibración y acciones de mantenimiento. & Cargas, Von Mises, desplazamiento y factor de seguridad.\\
RM-02 & Protección ambiental & Grado IP objetivo propuesto, sellos y drenaje/condensación. & Especificación de gabinete y revisión de juntas.\\
...
RM-05 & Ventilación y temperatura & Mantener componentes dentro de límites de hoja de datos. & Balance térmico y \textbf{[VALIDAR CON PRUEBA TÉRMICA]}.\\
RM-06 & Expansión & Reserva para terminales, comunicación o almacenamiento adicional. & Volumen libre y capacidad de riel/rack.\\
RM-07 & Soporte de cámara & Cobertura de zonas de interés con rigidez y torque suficientes. & Geometría de visión, cálculo de torque y desplazamiento.\\
...
\captionof*{figure}{\small\itshape Figura 66. Vista del Controlador SemafÃ³rico}
\addcontentsline{lof}{figure}{Figura 66. Vista del Controlador SemafÃ³rico}
\label{fig:66}
\end{center}
...
\section{Modos de operación y degradación segura}\label{modos-operacion-v2}

La salida difusa nunca constituye por sí sola una orden de lámpara. Primero se valida calidad de datos, vigencia, límites de fase y compatibilidad con el estado informado por el controlador. La Tabla 6.6 define el comportamiento esperado ante fallas.

\begin{longtable}{@{}p{0.18\linewidth}p{0.30\linewidth}p{0.24\linewidth}p{0.20\linewidth}@{}}
\toprule
...
Asistido por nube & Recomendación remota autenticada, vigente y dentro de rangos. & Combina o prioriza según regla de arbitraje y vuelve a validar límites. & Controlador semafórico/central autorizada.\\
Sin conectividad & Pérdida o expiración del enlace cloud. & Conserva configuración válida, almacena localmente y continúa con control local. & Controlador semafórico.\\
Sensado degradado & Video ausente, oclusión, baja confianza o variables fuera de rango. & Congela adaptación, genera alerta y solicita plan base. & Controlador semafórico.\\
Falla interna & Watchdog, sobretemperatura, alimentación inestable o error no recuperable. & Registra evento, inhibe recomendaciones y ejecuta apagado/reinicio controlado. & Controlador semafórico.\\
...
\textbf{Justificación del núcleo difuso.} La lógica difusa se selecciona porque ofrece reglas interpretables, incorpora conocimiento experto, opera con pocos datos iniciales, puede ejecutarse en el borde y permite verificar sistemáticamente sus límites. El aprendizaje por refuerzo y las redes CNN--LSTM pueden superar al control difuso en determinados escenarios, pero requieren conjuntos de entrenamiento, validación fuera de muestra y control de distribución que no están cerrados en esta tesis. En consecuencia, se conservan como módulos experimentales o futuros y no como condición para demostrar el objetivo general.
...
\section{Arquitectura de ciberseguridad propuesta}\label{arquitectura-ciberseguridad-v2}

La ciberseguridad se trata como una propiedad de arquitectura y no como una certificación alcanzada. El análisis considera activos, amenazas, impacto, probabilidad cualitativa, mitigación y evidencia posible dentro del alcance. La probabilidad deberá revisarse durante un piloto mediante inventario real, exposición de red y pruebas autorizadas.

\begin{landscape}
\footnotesize
\captionof*{table}{\small\bfseries Tabla 6.7. Modelo de amenazas y mitigaciones propuestas. (Creación propia)}
\addcontentsline{lot}{table}{Tabla 6.7. Modelo de amenazas y mitigaciones propuestas. (Creación propia)}
\label{tbl:amenazas-v2}
\begin{longtable}{@{}p{4.0cm}p{3.4cm}p{3.2cm}p{2.2cm}p{6.0cm}p{4.0cm}@{}}
\toprule
\textbf{Amenaza} & \textbf{Activo afectado} & \textbf{Impacto} & \textbf{Probabilidad} & \textbf{Medida de mitigación} & \textbf{Verificación en tesis}\\
\midrule
\endhead
...
Inyección de datos falsos & ICV, estados y alertas & Decisiones incorrectas & Media & Autenticación, integridad, sello temporal, detección de valores atípicos y comparación con estado local. & Pruebas con mensajes alterados o fuera de secuencia.\\
Pérdida de conectividad & Supervisión y coordinación & Pérdida de visibilidad global & Alta & Control local, búfer de registros, reintento con retroceso y plan base. & Caso D de validación y registro de transición.\\
Denegación de servicio & Enlace, broker o API & Indisponibilidad de nube & Media & Límites de tasa, colas, segmentación y operación local independiente. & Prueba controlada de indisponibilidad; no prueba de intrusión.\\
Compromiso de credenciales & Identidad de equipo/usuario & Suplantación & Media & Secretos no embebidos, rotación, revocación, MFA y almacenamiento seguro. & Revisión de arquitectura y procedimiento propuesto.\\
Actualización no autorizada & Software edge y configuración & Ejecución de código malicioso & Baja/Media & Firma de firmware, canal autenticado, control de versiones y rollback. & Flujo de actualización propuesto y validación de firma.\\
...
La medida arquitectónica principal es la separación entre \emph{recomendación} y \emph{acción final}. Aun si la nube, una cuenta o un mensaje fueran comprometidos, la recomendación debe atravesar validación de identidad, vigencia, rango y restricciones; posteriormente, el controlador semafórico conserva sus enclavamientos. Esta defensa reduce impacto, pero no reemplaza una auditoría, una prueba de penetración autorizada ni una evaluación de conformidad IEC 62443.

\section{Diseño del subsistema de nube y comunicación}\label{diseuxf1o-del-subsistema-de-nube-y-comunicaciuxf3n}

La infraestructura de Microsoft Azure constituye una capa de soporte para supervisión, almacenamiento, analítica y coordinación no crítica. El núcleo de decisión validado permanece en el procesamiento local. La arquitectura de nube se diseña con disponibilidad, escalabilidad y tolerancia a fallos, pero su indisponibilidad no debe impedir que el controlador semafórico continúe operando con su plan seguro.
...
\section{Protocolo de validación y criterio de evidencia}\label{protocolo-validacion-v2}

La validación se limita al entorno simulado y a la plataforma controlada. Para que la comparación sea defendible, todos los controladores deben usar la misma red, demanda, semilla, horizonte, periodo de calentamiento y conjunto de vehículos. Se recomienda ejecutar varias réplicas por caso y reportar media, dispersión e intervalo de confianza; una sola corrida no permite separar el efecto del controlador de la variabilidad estocástica.

\begin{longtable}{@{}p{0.12\linewidth}p{0.30\linewidth}p{0.27\linewidth}p{0.23\linewidth}@{}}
\toprule
\textbf{Caso} & \textbf{Estrategia} & \textbf{Propósito} & \textbf{Condición de aceptación}\\
\midrule
\endhead
A & Control semafórico fijo/base & Línea base reproducible. & Configuración documentada y común a todas las réplicas.\\
B & Control adaptativo local difuso & Evaluar el núcleo de tesis. & Mejora o no degradación según métricas y restricciones.\\
C & Difuso con recomendación de coordinación de nube (opcional) & Medir el beneficio adicional de la capa global. & Resultado separado del núcleo y recomendación validada localmente.\\
D & Pérdida de conectividad durante el caso B & Verificar autonomía y modo degradado. & Sin dependencia de respuesta cloud, sin estados incompatibles y con registro de evento.\\
E & Max-Pressure u otra referencia avanzada (opcional) & Establecer una cota comparativa con información ideal de SUMO. & No presentarlo como implementación realizable con el sensado actual.\\
\bottomrule
\end{longtable}
...
\centering
\textbf{Configurar red y demanda} \(\rightarrow\) \textbf{Ejecutar casos con semillas comunes} \(\rightarrow\)
\textbf{Exportar métricas SUMO}\\
\(\downarrow\) \hspace{4.5cm} \(\downarrow\)\\
\textbf{Verificar restricciones y fallas} \(\leftarrow\) \textbf{Comparar estadísticamente} \(\leftarrow\)
\textbf{Construir matriz de cumplimiento}
\end{minipage}}
\captionof*{figure}{\small\itshape Figura 7.A. Flujo de validación comparativa en SUMO. (Creación propia)}
\addcontentsline{lof}{figure}{Figura 7.A. Flujo de validación comparativa en SUMO. (Creación propia)}
\label{fig:flujo-validacion-v2}
\end{center}

\begin{longtable}{@{}p{0.28\linewidth}p{0.25\linewidth}p{0.39\linewidth}@{}}
\toprule
\textbf{Parámetro} & \textbf{Valor} & \textbf{Observación}\\
\midrule
\endhead
Escenario/red & \textbf{[IDENTIFICAR ARCHIVO .sumocfg]} & Indicar versión y área modelada.\\
Horizonte de simulación & \textbf{[COMPLETAR]} s & Separar calentamiento y periodo medido.\\
Número de réplicas & \textbf{[COMPLETAR]} & Recomendada pluralidad de semillas.\\
Semillas aleatorias & \textbf{[LISTAR]} & Deben ser comunes entre casos A--D.\\
Número de vehículos/demanda & \textbf{[INSERTAR VALOR OBTENIDO EN SUMO]} & Reportar flujo por acceso y distribución temporal.\\
Plan base & \textbf{[COMPLETAR CICLO, SPLIT Y OFFSET]} & Justificar con plan existente o Webster.\\
Intervalo de decisión difusa & \textbf{[COMPLETAR]} s & Coherente con latencia de sensado.\\
Evento de desconexión & \textbf{[COMPLETAR INSTANTE Y DURACIÓN]} & Aplicable al caso D.\\
Versión SUMO/Python & \textbf{[COMPLETAR]} & Necesaria para reproducibilidad.\\
\bottomrule
\end{longtable}
...
\subsection{Resultados comparativos y discusión}\label{resultados-comparativos-v2}

La Tabla 7.A debe completarse directamente con archivos de salida SUMO generados bajo el protocolo anterior. Para métricas que deben disminuir, la mejora porcentual se calcula como \((A-B)/A\times100\%\); para métricas que deben aumentar, como \((B-A)/A\times100\%\). No se reporta una mejora mientras las celdas permanezcan pendientes.

\begin{landscape}
\footnotesize
\captionof*{table}{\small\bfseries Tabla 7.A. Comparación del control base y el control adaptativo difuso. (Creación propia)}
\addcontentsline{lot}{table}{Tabla 7.A. Comparación del control base y el control adaptativo difuso. (Creación propia)}
\label{tbl:resultados-sumo-v2}
\begin{longtable}{@{}p{5.0cm}p{3.6cm}p{3.6cm}p{3.6cm}p{3.4cm}p{3.0cm}@{}}
\toprule
\textbf{Métrica} & \textbf{Caso A: base} & \textbf{Caso B: difuso} & \textbf{Variación absoluta} & \textbf{Mejora (\%)} & \textbf{Cumplimiento}\\
\midrule
\endhead
Tiempo promedio de espera (s/veh) & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Longitud promedio de cola (veh o m) & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Longitud máxima de cola (veh o m) & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Velocidad promedio (m/s) & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Throughput (vehículos completados) & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Número promedio de paradas (paradas/veh) & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Tiempo promedio de viaje (s) & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Índice de congestión de red (-) & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Combustible o emisiones & \textbf{[SUMO]} & \textbf{[SUMO]} & \textbf{[CALCULAR]} & \textbf{[CALCULAR]} & \textbf{[EVALUAR]}\\
Latencia de decisión local (ms) & No aplica & \textbf{[MEDIR]} & --- & --- & \textbf{[EVALUAR]}\\
\bottomrule
\end{longtable}
\end{landscape}

La discusión debe explicar no solo el signo de la mejora, sino también su estabilidad entre réplicas, el comportamiento por nivel de demanda, los casos donde el control difuso no supera al plan base y la relación entre reducción de demora y posibles efectos secundarios, como incremento de cola en el acceso perpendicular. La validez externa queda limitada por la calidad de la red OSM, la demanda sintética, el modelo de conductor y la ausencia de calibración con aforos reales.

\begin{longtable}{@{}p{0.28\linewidth}p{0.28\linewidth}p{0.36\linewidth}@{}}
\toprule
\textbf{Prueba de degradación} & \textbf{Resultado} & \textbf{Criterio}\\
\midrule
\endhead
Pérdida de nube durante operación & \textbf{[COMPLETAR CON CASO D]} & Continúa localmente o retorna al plan base sin estados incompatibles.\\
Datos de sensado inválidos & \textbf{[COMPLETAR]} & Inhibe adaptación y genera alerta.\\
Parámetro remoto fuera de rango & \textbf{[COMPLETAR]} & Rechazo y registro del evento.\\
Recomendación vencida/duplicada & \textbf{[COMPLETAR]} & Rechazo por vigencia o secuencia.\\
\bottomrule
\end{longtable}

\section{Validación del prototipo en entornos controlados}\label{validaciuxf3n-del-prototipo-en-entornos-controlados}

La validación en entorno controlado utiliza una plataforma web interactiva que integra SUMO con visualización geoespacial. Esta plataforma permite observar estados, ejecutar escenarios y exportar datos; por sí sola no demuestra una mejora de desempeño. La evidencia cuantitativa corresponde a los archivos y tablas comparativas definidos en la sección anterior.
...
\section{Matriz de cumplimiento de requerimientos}\label{cumplimiento-requerimientos-v2}

La matriz de cumplimiento distingue entre diseño documentado y validación cuantitativa. ``Cumple parcialmente'' significa que existe una propuesta o análisis, pero falta una prueba, cálculo o integración necesaria para cerrar el requerimiento.

\begin{landscape}
\footnotesize
\captionof*{table}{\small\bfseries Tabla 7.B. Cumplimiento de requerimientos del sistema. (Creación propia)}
\addcontentsline{lot}{table}{Tabla 7.B. Cumplimiento de requerimientos del sistema. (Creación propia)}
\label{tbl:cumplimiento-v2}
\begin{longtable}{@{}p{2.2cm}p{4.0cm}p{5.0cm}p{3.5cm}p{7.0cm}@{}}
\toprule
\textbf{Requisito} & \textbf{Subsistema} & \textbf{Prueba o análisis} & \textbf{Resultado} & \textbf{Cumplimiento/observación}\\
\midrule
\endhead
R-F01 & Sensado & Visión/SUMO & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-F02 & Procesamiento local & Casos de ICV y sensibilidad & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-F03 & Plataforma & Inspección de mensajes e históricos & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-C01 & Control & Caso B y desconexión & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-C02 & Arquitectura & Revisión de frontera funcional & Controlador conserva autoridad & Cumple a nivel de diseño; integración comercial pendiente.\\
R-C03 & Control & Barrido de reglas y saturaciones & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-C04 & Control & Falla de sensado & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-COM01 & Comunicación & Publicación de telemetría & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-COM02 & Comunicación/seguridad & Parámetros inválidos y vencidos & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-NUB01 & Nube & Dashboard, histórico y alertas & \textbf{[COMPLETAR]} & Cumple parcialmente hasta demostrar disponibilidad y retención.\\
R-SEC01 & Ciberseguridad & Configuración TLS/autenticación & \textbf{[COMPLETAR]} & Arquitectura propuesta; auditoría fuera del alcance.\\
R-SEC02 & Ciberseguridad & Modelo de amenazas & Tabla 6.7 & Cumple a nivel de análisis.\\
R-SEC03 & Ciberseguridad & Inspección de logs & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-SEC04 & Ciberseguridad & Revisión de roles y autoridad & Separación documentada & Cumple a nivel de arquitectura.\\
R-ELEC01 & Energía & Esquema y coordinación & \textbf{[VALIDAR CON CÁLCULO]} & Cumple parcialmente.\\
R-ELEC02 & Energía & Balance de potencia/autonomía & \textbf{[VALIDAR CON CÁLCULO]} & Cumple parcialmente.\\
R-ELEC03 & Electrónica & PCB/distribución/diagnóstico & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-MEC01 & Mecánico & CAD y acceso & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-MEC02 & Mecánico & Gabinete, sellos y material & \textbf{[COMPLETAR]} & Certificación IP no ejecutada.\\
R-MEC03 & Mecánico/sensado & Cobertura, torque y resistencia & \textbf{[COMPLETAR]} & \textbf{[EVALUAR]}\\
R-OP01 & Operación & Procedimiento de mantenimiento & \textbf{[AGREGAR EVIDENCIA]} & Cumple parcialmente.\\
R-VAL01 & Validación & Casos A y B & \textbf{[RESULTADOS SUMO]} & Pendiente de resultados reproducibles.\\
R-VAL02 & Validación & Tabla 7.A & \textbf{[RESULTADOS SUMO]} & Pendiente.\\
R-VAL03 & Validación & Caso D & \textbf{[RESULTADO DE DESCONEXIÓN]} & Pendiente.\\
R-COST01 & Costos & CAPEX/OPEX & Capítulo 7 & Actualizar fuentes y supuestos antes de sustentar.\\
\bottomrule
\end{longtable}
\end{landscape}
...
\begin{enumerate}
\item La problemática se delimitó como una limitación de ingeniería: los planes fijos o con adaptación limitada no representan adecuadamente variaciones direccionales de demanda, mientras que una arquitectura exclusivamente remota introduce dependencia de conectividad. Esto justifica un módulo local complementario y no el reemplazo directo del controlador semafórico.

\item El diseño conceptual estableció una cadena trazable entre necesidades, requerimientos, funciones y alternativas. La evaluación ponderada seleccionó el concepto cámara + edge + ICV + control difuso + nube segura + gabinete modular, principalmente por autonomía local, interpretabilidad y compatibilidad con una validación reproducible.

\item El diseño electrónico definió las funciones de sensado, procesamiento, comunicación, alimentación, protección, respaldo y diagnóstico. Su cierre para fabricación requiere completar o confirmar el balance de potencia, la autonomía del respaldo, la coordinación de protecciones y las interfaces con el controlador objetivo mediante los cálculos señalados en el documento.

\item El diseño mecánico incorporó gabinete, distribución interna, ventilación, acceso de mantenimiento y soporte de cámara como partes necesarias del desempeño. Los análisis CAD y estructurales constituyen verificación de diseño; la clasificación IP, resistencia al vandalismo y comportamiento térmico deben confirmarse mediante prototipo y ensayos.

\item El control difuso local se definió como núcleo por su interpretabilidad y capacidad de operar en el borde. Las entradas, membresías, reglas, inferencia, defuzzificación y saturaciones permiten revisar el comportamiento antes de emitir una recomendación. Ante falla de sensado o del módulo, la arquitectura inhibe la adaptación y conserva el plan seguro del controlador.

\item La nube se delimitó como soporte para telemetría, históricos, alertas, coordinación y distribución autorizada de parámetros. La ciberseguridad propuesta incorpora autenticación, cifrado, roles, validación, registros y mínimo privilegio; estas medidas son criterios de diseño y no equivalen a una auditoría o certificación.

\item Se definió un protocolo SUMO que compara control fijo y control difuso bajo condiciones comunes e incluye un caso de pérdida de conectividad. La conclusión cuantitativa sobre mejora de espera, colas, velocidad, throughput, paradas, viaje, ICV y emisiones queda condicionada a completar la Tabla 7.A con resultados reproducibles: \textbf{[COMPLETAR CON RESULTADOS SUMO]}. Mientras dicha evidencia no esté incorporada, solo puede afirmarse que existe un entorno de validación diseñado.

\item El análisis económico preliminar separa CAPEX y OPEX y permite identificar los rubros dominantes. Antes de defender una cifra final deben actualizarse cotizaciones, fecha, tipo de cambio, consumo real y prorrateo de servicios de nube; la certificación, obra civil y piloto deben presupuestarse como etapas posteriores.

\item La principal limitación del trabajo es la ausencia de fabricación, calibración con aforos reales, integración con un controlador comercial certificado, prueba vial y auditoría de ciberseguridad. Estas limitaciones no invalidan el diseño, pero restringen la generalización de los resultados a las condiciones simuladas y a los supuestos documentados.

\item La arquitectura resultante constituye una base técnica para un prototipo de laboratorio y un piloto progresivo. La condición de avance debe ser demostrar cada requerimiento mandatorio en banco, preservar la autoridad final del controlador y obtener aprobación normativa e institucional antes de cualquier intervención en vía pública.
\end{enumerate}

\section*{Recomendaciones}

\begin{itemize}
\item Fabricar un prototipo del módulo y verificar consumo, temperatura, respaldo, comunicaciones y watchdog en un banco de pruebas.
\item Probar la cámara con video real de Lima en día, noche, contraluz, lluvia o neblina costera, y medir precisión de conteo, cola y velocidad.
\item Calibrar la demanda y el comportamiento vehicular de SUMO con aforos reales; ejecutar múltiples semillas y publicar archivos de configuración y salida.
\item Integrar el módulo a un controlador semafórico comercial en un banco hardware-in-the-loop, sin conexión directa a lámparas durante la etapa inicial.
\item Ejecutar pruebas autorizadas de ciberseguridad, gestión de certificados, rotación de credenciales, restauración de backups y respuesta ante incidentes.
\item Revisar normativa vial, seguridad funcional, compatibilidad electromagnética, protección ambiental y procedimientos municipales aplicables.
\item Realizar un piloto controlado en una intersección o recinto cerrado, con plan de reversión, supervisión humana y métricas acordadas.
\item Mantener RL, CNN--LSTM y optimización metropolitana como líneas futuras hasta disponer de datos, validación fuera de muestra y criterios de aceptación.
\item Extender el estudio de una intersección a un corredor solo después de demostrar estabilidad local, interoperabilidad y beneficio estadísticamente sustentado.
\end{itemize}
